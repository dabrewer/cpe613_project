...
##rc = np.array([rgb(np.min(v), np.max(v), i)[0] for i in v])
##gc = np.array([rgb(np.min(v), np.max(v), i)[1] for i in v])
##bc = np.array([rgb(np.min(v), np.max(v), i)[2] for i in v])
...
ax.voxels(x, y, z, voxels2, facecolors=colors2, edgecolor='k', cmap='bwr')
